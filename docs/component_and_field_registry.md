...
| `MindComponent` | `known_runes:Array[StringName]`, `insight:Dictionary{StringName:int}`, `faction_reputations:Dictionary{int:float}`, `language_fluency:Dictionary{StringName:int}`, `grimoire:Dictionary{StringName:CompiledSpell}` (bound Action_IDs), `active_spell:StringName` (what the cast key fires) |
...
- `CompiledSpell` (Sprint 4) = a validated spell produced ONLY by `SpellCompilerSystem.compile`:
  ...
  caps_applied:Array[StringName] }`. Nothing else may construct one: the geometric caps are
  anti-crash rules, and a second construction path is a second place to forget them.
...
- `ChunkData` = `chunk_id:Vector3i`, `state:LoD`, `biome_tag:StringName`,
  ...
  `wealth_materialized:bool`, `tile_map_dirty:bool`,
...
- Tags are `StringName` in `ChemistryComponent.active_tags` (state/chemistry) — e.g.
  ...
  Sprint 4 adds `&"Spores"`, `&"Volatile_Gas"`, `&"Water"`, `&"Explosion"`, `&"Steam"`,
